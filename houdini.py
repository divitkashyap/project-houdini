...
from qiskit_optimization.applications import Maxcut from qiskit.circuit.library import QAOAAnsatz
...
gammas = np.random.uniform(0, 2*np.pi, qaoa_reps) parameter_values = [*betas, *gammas]
...
set_a): cut_edges.append(edge)
# Check if this partition has more cuts
if len(cut_edges) > max_cuts: max_cuts = len(cut_edges)
...
print(f"Best␣partition:␣Set␣A:␣{best_partition[0]},␣Set␣B:␣{best_partition[1]}") print(f"Cut␣edges:␣{best_partition[2]}")